...
- **Cross-encoder reranking** (retrieval quality): `retrieval/reranker.py` reorders a larger cosine-similarity candidate pool with a `sentence-transformers` `CrossEncoder` (`RERANK_ENABLED`/`RERANK_MODEL`/`RERANK_CANDIDATE_POOL`). The off-topic gate reads a `top_match_score` captured from the pool *before* reranking, so `MIN_RELEVANCE_SCORE`'s calibration (Phase F follow-up) is unaffected either way — verified live against the same boundary queries used to calibrate it originally. `evaluate_rag.py` hit-rate stayed 100% (MRR 0.979, down from 1.0 — one case moved from rank 1 to rank 2, still within top_k). **Turned off in production (2026-06-25):** Render's free-tier CPU added ~9-10s to every `/ask` call running the cross-encoder's per-candidate forward passes (measured live via `X-Process-Time-Ms`, with off-topic queries that skip the Groq call entirely taking just as long — confirming the rerank/embedding pipeline, not Groq, was the bottleneck). `render.yaml` now sets `RERANK_ENABLED=false` as a production-only override; `config.py`'s default (`true`) is unchanged for local dev. No measured quality cost on this catalog — disabling it actually restores MRR to 1.0 per the line above.
