...
- **Gotcha hit during deploy:** the Render Blueprint did not auto-sync `render.yaml`'s `CORS_ORIGINS` change on a plain `git push` to `main` — had to update the value directly in the Render dashboard's Environment tab to trigger the redeploy. If you change `render.yaml` env vars again, verify the dashboard actually picked it up (check the Events/Deploys tab) rather than assuming auto-sync fired.
- Verified live end-to-end: CORS preflight returns the right `Access-Control-Allow-Origin`, and an actual cross-origin POST from the Vercel origin to `/ask` returns a correct grounded answer with sources.
