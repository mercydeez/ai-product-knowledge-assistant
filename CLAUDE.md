...
**`RERANK_ENABLED` defaults to `true` in `config.py` but is forced to `false` in production via `render.yaml`.** Render's free-tier CPU couldn't run the cross-encoder's per-candidate forward passes without adding ~9-10s to every `/ask` call (confirmed live: off-topic queries, which skip the Groq call entirely, took just as long, isolating the rerank/embedding pipeline — not Groq — as the bottleneck). This is a deliberate local/production split, not a bug — local dev still exercises reranking by default. Don't "fix" `render.yaml` back to match `config.py`'s default without re-measuring latency on the free tier first.
...
**`/ask` and `/ask/stream` are rate-limited per client IP (`RATE_LIMIT_ASK`, default `20/minute`); `/health` is not.** `src/api/rate_limit.py` holds a standalone `slowapi` `Limiter` (its own module so both `app.py` and `routes.py` can import it without a circular import — `app.py` already imports `router` from `routes.py`). `app.py` registers a custom `RateLimitExceeded` handler returning `{"detail": "Rate limit exceeded: ..."}` (429) so the shape matches the rest of the API instead of slowapi's default `{"error": ...}`. slowapi's `@limiter.limit(...)` decorator requires the route's `Request` parameter to be named exactly `request` (it locates it by name via `inspect.signature`, not by type) — this is why `routes.py`'s `/ask` handlers take `payload: AskRequest, request: Request` rather than the `request: AskRequest, http_request: Request` naming used before rate limiting was added. `AskRequest.question` (`src/api/schemas.py`) also has `max_length=500` — without it, a single oversized question would still pay full embedding + Groq cost under the limit, defeating its purpose.

**The rate limiter's per-IP keying only works if the deployed process trusts Render's reverse proxy.** `slowapi`'s `get_remote_address()` (used as `rate_limit.py`'s `key_func`) reads `request.client.host` — the direct TCP peer, not `X-Forwarded-For` — and uvicorn only rewrites that from `X-Forwarded-For` when `--proxy-headers` is on *and* the peer is in `--forwarded-allow-ips` (default `127.0.0.1`). Render's edge proxy connects from a different IP, so without `Dockerfile`'s `CMD` passing `--proxy-headers --forwarded-allow-ips='*'`, every visitor would appear to share Render's proxy IP and the "per-IP" limit silently becomes one global limit shared by everyone hitting the live demo. Confirmed via `uvicorn.config.Config`'s actual defaults (`proxy_headers=True`, `forwarded_allow_ips` falls back to `127.0.0.1`), not just docs — if you ever swap rate-limit libraries or hosts, re-verify this assumption rather than trusting it carried over.
...
- `requirements.txt` is trimmed to the packages actually imported by `src/` (`fastapi`, `uvicorn`, `pydantic`, `python-dotenv`, `sentence-transformers`, `chromadb`, `numpy`, `groq`, `slowapi`). If you add a new direct import, add it here too — don't let it drift back into a full env dump.
- **`slowapi`'s in-memory rate-limit storage is process-global, not per-test.** `tests/test_api.py` calls `limiter.reset()` (imported from `src/api/rate_limit.py`) in `setUp()` so each test method starts with a clean quota for the shared `Limiter` instance — without it, later tests would inherit quota consumed by earlier ones (or vice versa) and become order-dependent.
